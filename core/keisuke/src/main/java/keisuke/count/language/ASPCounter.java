...
public class ASPCounter extends GeneralStepCounter {
	...
	public ASPCounter(final String type) {
		...
		if (type.equals("ASP")) {
			...
			LogUtil.errorLog("Not ASP nor ASP.NET, but unexpected type: " + type);
			throw new RuntimeException("Not ASP nor ASP.NET");
		}
	...
	protected String searchScriptStart(final String line) {
		...
		if (start.toUpperCase().startsWith(SCRIPT_TAG)) {
			...
			if (this.currentLang() != null) {
				...
				//LogUtil.debugLog("Directive Lang : " + defaultLang.languageName());
			}
		}
		if (this.currentLang() == null) {
		...
		//LogUtil.debugLog("Current Lang : " + this.currentLang().languageName());
	...
	protected String findScriptTag(final String tag, final String line) {
		...
		//LogUtil.debugLog("Get Tag :" + target);
		...
		for (int i = 0; i < checkArray.length; i++) {
			...
			for (int j = 0; j < targetArray.length; j++) {
				...
				if (targetStr.equals(checkStr)) {
					checkOK = true;
					//LogUtil.debugLog("Match : " + checkStr);
					break;
				}
			}
			if (!checkOK) {
		...
		for (int j = 0; j < targetArray.length; j++) {
			...
			if (targetStr.startsWith(TAG_ATTR_LANG)) {
				...
				//LogUtil.debugLog("Language : " + tagLang);
				break;
			}
		}
		return target;
